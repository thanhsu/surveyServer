...
public class ServiceInit extends MicroServiceVerticle {
	...
	private Future<JsonObject> depositHandler(JsonObject msg) {
		...
		Long amount = msg.getLong(FieldName.AMOUNT);
		...
		PaypalConnection.getInstance().createPaymentSDK(tranID, "0", total.toString(), x,
	...
	private Future<JsonObject> withdrawHandler(JsonObject msg) {
		Future<JsonObject> lvResult = Future.future();
		...
		PaypalConnection.getInstance().createPayout(tranID, ccy, String.valueOf(amount), userType, tagetUser, remark)
				.setHandler(handler -> {
					if (handler.succeeded()) {
						lvResult.complete(handler.result());
					} else {
