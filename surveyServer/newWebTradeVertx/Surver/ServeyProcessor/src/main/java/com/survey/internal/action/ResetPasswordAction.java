...
public class ResetPasswordAction extends BaseSurveyInternalAction {
	...
	private void resetPasswordFirst() {
		...
		JsonObject queryUserInfo = new JsonObject().put(FieldName.ACTION, "userinfo").put(FieldName.USERNAME,
				lvUsername);
						...
						VertxServiceCenter.getEventbus().<JsonObject>send(record.getLocation().getString("endpoint"),
								queryUserInfo, res -> {
									if (res.succeeded()) {
										...
										if (msg.getString(FieldName.CODE).equals(CodeMapping.U0000)) {
											// Check Email
											try {
												String email = msg.getString(FieldName.EMAIL);
												...
												if (email.equals(lvEmail)) {
													...
													String link = ProcessorInit.mvConfig.getString("SurveyHost")
															+ "/resetpassword/1?" + lvUsername + "=" + token;
													// Send link to Email
													if (SurveyMailCenter.getInstance().sendEmail(lvEmail,
															"ResetPassword-ISurvey", "",
															"Reset password your account by link: " + link
																	+ "\n Your link expired at: " + lvExprired)) {
														reset1st.put(FieldName.CODE, CodeMapping.C0000.toString())
																.put(FieldName.MESSAGE, "Check your email inbox");
														// Store token
														storeResetToken(lvUsername, token, lvNow);
													} else {
														reset1st.put(FieldName.CODE, CodeMapping.U8888.toString())
																.put(FieldName.MESSAGE, CodeMapping.U8888.value());
													}

													response.complete(reset1st);
												} else {
													reset1st.put(FieldName.CODE, CodeMapping.U7777.toString())
															.put(FieldName.MESSAGE, CodeMapping.U7777.value());
												}
											} catch (Exception e) {
						...
						resultHandler -> {
							if (resultHandler.succeeded() && resultHandler.result() != null) {
								...
								VertxServiceCenter.getEventbus().<JsonObject>send(
										record.getLocation().getString("endpoint"), resetPasswordRequest, res -> {
											messageResponse = new JsonObject();
											if (res.succeeded() & res.result() != null) {
												response.complete(res.result().body());
											} else {
												messageResponse.put(FieldName.CODE, CodeMapping.C1111.toString())
														.put(FieldName.MESSAGE, "Request Error");
