			...
			if (balance.succeeded()) {
				...
				if (Utils.checkWithdraw(amount, etheAmount)) {
					...
					lvUtilsDao.retrieveEthePointValue(false).setHandler(point -> {
						try {
							double exchangeRate = Double
									.parseDouble(point.result().getJsonArray(FieldName.DATA).getJsonObject(0)
											.getString(FieldName.VALUE))
									/ Double.parseDouble(point.result().getJsonArray(FieldName.DATA).getJsonObject(0)
													...
													new JsonObject().put(FieldName.CODE, CodeMapping.C0000.toString())
															.put(FieldName.DATA,
																	new JsonObject().put("success", true)
																			...
																			.put(FieldName.TRANID, id).put(FieldName.TEMPAMOUNT, exchangeRate*amount)));
										} else {
											this.response.complete(
									...
									.setHandler(handler -> {
										if (handler.succeeded()) {
											...
											sendCashWithdraw(username,
													String.valueOf(
															exchangeRate * getMessageBody().getInteger(FieldName.AMOUNT)),
