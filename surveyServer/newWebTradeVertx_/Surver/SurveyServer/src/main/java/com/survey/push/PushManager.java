...
@ManagedService(path = "/push/{module}/{action: [a-zA-Z]+}/{session: [a-zA-Z0-9]+}", interceptors = {
...
public class PushManager {
	...
	public Object onReady(final AtmosphereResource resource) {
		...
		// ITradeAtmosphereAPI.getInstance().subscribeTopic(resource, mvClientID,
		// pTopics);
	...
	public PushResponseJSONBean onTopicSubscribe(AtmosphereResource resource) {
		...
		resource.getResponse().addHeader("Access-Control-Allow-Origin", "*");
		resource.getResponse().addHeader("Origin", "*");
		return lvResBean;
	}
	...
	public void postEncoding(AtmosphereResource resource) {
		resource.getResponse().setCharacterEncoding("UTF-8");
		// if (resource.getRequest().getHeader(HeaderConfig.X_ATMOSPHERE_TRANSPORT) !=
		// null) {
	...
	public JsonObject onDisconnect(AtmosphereResourceEvent event) {
		...
		try {
			...
			if (event.isCancelled()) {
				...
				Log.println("Browser " + uuid
						+ "@Suspend( contentType = MediaType.APPLICATION_JSON, period = MAX_SUSPEND_MSEC ) closed the connection.",
						Log.DEBUG_LOG);
