...
import com.survey.utils.TextUtil;
...
public class ITradeAtmosphereAPI {
	...
	public boolean pushTopic(final String module, final String clientId, final String topic, Object o) {
		...
		if (lvBroadcaster == null) {
			Log.println("[ITradeAtmosphere] Client ID " + clientId + " not found in list stock . Removed",
					Log.ACCESS_LOG);
			return false;
		} else {
	...
	// public boolean pushTopicWatchList(final String module, final String action,
	...
	// public boolean pushTopicNotification(final String module, final String
	// action, final String
	// session, Object o) {
	// HashMap<String, String> data = new HashMap<>();
	// data.put(CONTRACT_SESSION, session);
					...
					if (data.succeeded() && data.result() != null) {
						...
						result.result().replace(clientID, lvITradeUser, res -> {
							if (res.succeeded()) {
								...
								lvFuture.complete();
								Log.print("UPDATE new topic FAILED: " + clientID + " Topic: " + pTopics.toString()
										+ " . Cause: " + res.cause());
							}
						});
					} else {
						...
						result.result().put(clientID, lvITradeUser, res -> {
							if (res.succeeded()) {
								...
								lvFuture.fail("Error. Cause:" + res.cause());
								lvFuture.complete();
								Log.print("CREATE new topic FAILED: " + clientID + " Topic: " + pTopics.toString()
										+ " .Cause: " + res.cause());
							}
						});
					}
		...
		mvSharedData.getClusterWideMap(USER_SESSION_SHARE, result -> {
			result.result().putIfAbsent(clienid, uuid, res -> {
				if (res.succeeded()) {
					...
					Log.print("FAILED Shared User: " + clienid + " with UUID Session: " + uuid + " . CAUSE: "
							+ res.cause());
				}
			});
		});
	...
	private boolean broadcast(Broadcaster broadcaster, Object jsonObject, String action) {
		// ItradeResponseJSONBean lvResponseJSONBean = new
		// ItradeResponseJSONBean(action, 0, jsonObject);
